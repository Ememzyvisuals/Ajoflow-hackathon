...
export default function SignUpPage() {
  ...
  const { register, handleSubmit, getValues, formState: { errors } } = useForm<SignUpInput>({
  ...
  if (success || magicSent) return (
    <div className="w-full max-w-sm text-center">
      ...
      <button
        onClick={() => { setSuccess(false); setMagicSent(false); }}
        className="text-primary text-sm font-medium mt-5 hover:underline"
      >
        ← Back to sign up
      </button>
    </div>
  ...
  return (
    <div className="w-full max-w-sm">
      ...
      <div className="text-center mb-6">
        ...
        <p className="text-text-secondary text-sm">Join AjoFlow and start managing your Ajo groups.</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-border p-6 shadow-sm">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            ...
            <input {...register("fullName")} placeholder="John Emmanuel" className="form-input" autoComplete="name" />
          ...
          <div>
            ...
            <div className="flex gap-2">
              <span className="flex items-center gap-1 border border-border rounded-lg px-3 text-sm text-text-secondary bg-gray-50 whitespace-nowrap">
                🇳🇬 +234
              </span>
              <input {...register("phone")} type="tel" placeholder="801 234 5678" className="form-input flex-1" autoComplete="tel" />
            </div>
          </div>

          <div>
            <label className="block text-sm font-medium text-text mb-1.5">Create password</label>
          ...
          <p className="text-xs text-text-secondary leading-relaxed">
            ...
            <Link href="/terms" target="_blank" className="text-primary font-medium underline underline-offset-2 hover:text-primary/80">Terms of Service</Link>{" "}and{" "}<Link href="/privacy" target="_blank" className="text-primary font-medium underline underline-offset-2 hover:text-primary/80">Privacy Policy</Link>
          </p>
        ...
        <div className="relative my-5">
          ...
          <div className="relative flex justify-center">
            <span className="bg-white px-3 text-xs text-text-secondary">or continue with</span>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 border border-border rounded-xl py-2.5 text-sm font-medium text-text hover:bg-gray-50 transition-colors disabled:opacity-60"
          >
            {googleLoading ? (
