...
export default function SignUpPage() {
  ...
  async function onSubmit(data: FormData) {
    ...
    setLoading(false);
    if (result.success) {
      const d = result.data as { requiresConfirmation?: boolean } | undefined;
      d?.requiresConfirmation ? setSuccess(true) : router.push("/onboarding");
    } else {
  ...
  if (success) return (
    <div className="w-full max-w-sm text-center">
      ...
      <p className="text-text-secondary text-sm">Confirmation link sent. Click it to activate your account.</p>
    </div>
  );
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-text mb-1.5">
              Phone number <span className="text-text-secondary text-xs">(optional)</span>
            </label>
            <div className="flex gap-2">
              <span className="flex items-center gap-1 border border-border rounded-lg px-3 text-sm text-text-secondary bg-gray-50">🇳🇬 +234</span>
          ...
          <div>
            ...
            <div className="relative">
              <input {...register("password")} type={showPw ? "text" : "password"} placeholder="••••••••" className="form-input pr-10" autoComplete="new-password" />
